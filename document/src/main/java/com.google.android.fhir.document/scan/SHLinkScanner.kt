 ...
 * Implementations of this interface are responsible for handling the scanning process of SHL QR codes
 * and providing relevant data through callbacks.
 *
 ...
 *
 * ## Scanning Process:
 * A SHL QR code contains a Smart Health Link. The scanning implementation should
 * extract this link and process it to provide an initialised [SHLinkScanData] object.
 *
 * ## Callbacks:
 * - `successCallback`: Invoked when the SHL QR code is successfully scanned, providing an
 *   initialised [SHLinkScanData] object.
 * - `failCallback`: Invoked when an error occurs during the scanning process, providing an
 *   [Error] object with details about the failure.
 */
interface SHLinkScanner {

  /**
   * Scans a SHL QR code and returns an initialised [SHLinkScanData] object
   * through the success callback or reports an error through the fail callback.
   *
