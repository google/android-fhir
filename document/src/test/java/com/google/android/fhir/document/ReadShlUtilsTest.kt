...
class ReadShlUtilsTest {
  ...
  fun substringSuccessfullyExtractsUrl() {
    Assert.assertEquals(readShlUtils.extractUrl(url), extracted)
  }

  @Test
  fun decodeFunctionProperlyDecodesUrl() {
    val decodedWithFunction = String(readShlUtils.decodeUrl(readShlUtils.extractUrl(url)))
    Assert.assertEquals(decodedWithFunction, decodedBase64)
  }

  @Test
  fun extractVerifiableCredentialCorrectlyExtractsAToken() {
    val extractedVCWithFunc = readShlUtils.extractVerifiableCredential(verifiableCredential)
  ...
  fun jwtsCanBeDecodedAndDecompressedIntoData() {
    ...
    val decoded = readShlUtils.decodeAndDecompressPayload(jwt)
  ...
  fun canDecodeJWE() {
    ...
    println(readShlUtils.decodeShc(jwe, "VmFndWVseS1FbmdhZ2luZy1QYXJhZG94LTA1NTktMDg"))
  }
}
