...
class R4FhirR4SyncWorkerTest {
  ...
  class FailingPeriodicSyncWorkerR4(appContext: Context, workerParams: WorkerParameters) :
    R4FhirSyncWorker(appContext, workerParams) {

    override fun getFhirEngine(): R4FhirEngine = TestingUtils.TestFhirEngineImpl
    override fun getDataSource(): R4DataSource = TestingUtils.TestFailingDatasource
    override fun getDownloadWorkManager(): R4DownloadWorkManager =
      TestingUtils.TestDownloadManagerImpl()
    override fun getConflictResolver() = AcceptRemoteConflictResolver
  }

  class FailingPeriodicSyncWorkerWithoutDataSourceR4(
    appContext: Context,
    ...
    override fun getDataSource(): R4DataSource? = null
    override fun getConflictResolver() = AcceptRemoteConflictResolver
  }
  ...
  fun fhirSyncWorker_successfulTask_resultSuccess() {
    ...
    val worker =
      TestListenableWorkerBuilder<PassingPeriodicSyncWorkerR4>(
          context,
  ...
  fun fhirSyncWorker_failedTaskWithZeroRetires_resultShouldBeFail() {
    val worker =
      TestListenableWorkerBuilder<FailingPeriodicSyncWorkerR4>(
          context,
  ...
  fun fhirSyncWorker_nullDataSource_resultShouldBeFail() {
    val worker =
      TestListenableWorkerBuilder<FailingPeriodicSyncWorkerWithoutDataSourceR4>(
          context,
          inputData = Data.Builder().putInt(MAX_RETRIES_ALLOWED, 2).build(),
          runAttemptCount = 2
        )
