...
import com.google.android.fhir.r4.sync.SyncJob
import com.google.android.fhir.r4.sync.SyncWorkType
...
class R4SyncJobTest {
  ...
  private val database = mock<R4Database>()
  private val r4DataSource = mock<R4DataSource>()

  @get:Rule var instantExecutorRule = InstantTaskExecutorRule()

  private lateinit var syncJob: SyncJob
  private lateinit var mock: MockedStatic<R4FhirEngineProvider>

  @Before
  fun setup() {
    fhirEngine = R4FhirEngineImpl(database, context)
    syncJob = R4Sync.basicSyncJob(context)
    ...
    mock = Mockito.mockStatic(R4FhirEngineProvider::class.java)
    whenever(R4FhirEngineProvider.getDataSource(anyOrNull())).thenReturn(r4DataSource)
  }
  ...
  fun `should poll accurately with given delay`() = runBlockingTest {
    val worker = PeriodicWorkRequestBuilder<TestSyncWorkerR4>(15, TimeUnit.MINUTES).build()
  ...
  fun `should run synchronizer and emit states accurately in sequence`() = runBlockingTest {
    ...
    whenever(r4DataSource.download(any()))
  ...
  fun `should run synchronizer and emit  with error accurately in sequence`() = runBlockingTest {
    whenever(database.getAllLocalChanges()).thenReturn(listOf())
    whenever(r4DataSource.download(any())).thenThrow(IllegalStateException::class.java)
  ...
  fun `sync time should update on every sync call`() = runBlockingTest {
    val worker1 = PeriodicWorkRequestBuilder<TestSyncWorkerR4>(15, TimeUnit.MINUTES).build()
    ...
    val worker2 = PeriodicWorkRequestBuilder<TestSyncWorkerR4>(15, TimeUnit.MINUTES).build()
  ...
  fun `number of resources loaded equals number of resources in TestDownloaderImpl`() =
      runBlockingTest {
    whenever(database.getAllLocalChanges()).thenReturn(listOf())
    whenever(r4DataSource.download(any())).thenReturn(Bundle())
    ...
    verify(r4DataSource, times(3)).download(any())
  }

  @Test
  fun `should fail when there data source is null`() = runBlockingTest {
    whenever(R4FhirEngineProvider.getDataSource(anyOrNull())).thenReturn(null)
    whenever(database.getAllLocalChanges()).thenReturn(listOf())
    whenever(r4DataSource.download(any()))
      .thenReturn(Bundle().apply { type = Bundle.BundleType.SEARCHSET })
    ...
    assertThat(result).isInstanceOf(com.google.android.fhir.r4.sync.Result.Error::class.java)
    assertThat(
        (result as com.google.android.fhir.r4.sync.Result.Error).exceptions.first().exception
      )
