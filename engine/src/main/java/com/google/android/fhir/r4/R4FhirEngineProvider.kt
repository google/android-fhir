...
object R4FhirEngineProvider {
  ...
  fun init(fhirEngineConfiguration: FhirEngineConfiguration) {
    check(R4FhirEngineProvider.fhirEngineConfiguration == null) {
      "R4FhirEngineProvider: FhirEngineConfiguration has already been initialized."
    }
    R4FhirEngineProvider.fhirEngineConfiguration = fhirEngineConfiguration
  }
   ...
   */
  @Synchronized
  fun getInstance(context: Context): R4FhirEngine {
    return getOrCreateFhirService(context).fhirEngine
  }

  @Synchronized
  @JvmStatic // needed for mockito
  internal fun getDataSource(context: Context): R4DataSource? {
    return getOrCreateFhirService(context).remoteDataSource
  }

  @Synchronized
  private fun getOrCreateFhirService(context: Context): R4FhirServices {
    if (fhirServices == null) {
      ...
      fhirServices =
        R4FhirServices.builder(context.applicationContext)
          .apply {
