...
interface UploadStrategy {
  ...
  suspend fun handleUploadResponse(response: Pair<LocalChangeToken, Resource>)
}

internal class PostUploadStrategy(private val database: Database) : UploadStrategy {
  override suspend fun handleUploadResponse(response: Pair<LocalChangeToken, Resource>) {
...
internal class PutUploadStrategy(private val database: Database) : UploadStrategy {
  override suspend fun handleUploadResponse(response: Pair<LocalChangeToken, Resource>) {
    database.deleteUpdates(response.first)
    when (response.second) {
      is Bundle -> updateVersionIdAndLastUpdated(response.second as Bundle)
      else -> updateVersionIdAndLastUpdated(response.second)
    }
  }
