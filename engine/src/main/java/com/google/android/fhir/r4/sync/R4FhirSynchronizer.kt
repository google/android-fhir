...
import com.google.android.fhir.r4.sync.download.R4DownloaderImpl
...
internal class R4FhirSynchronizer(
  context: Context,
  private val fhirEngine: R4FhirEngine,
  private val dataSource: R4DataSource,
  private val downloadManager: R4DownloadWorkManager,
  private val uploader: Uploader =
    R4BundleUploader(dataSource, TransactionBundleGenerator.getDefault()),
  private val r4Downloader: R4Downloader = R4DownloaderImpl(dataSource, downloadManager),
  ...
  private suspend fun download(): Result {
    ...
    fhirEngine.syncDownload(conflictResolver) {
      flow {
        r4Downloader.download(it).collect {
          when (it) {
