...
object LuceneSearcher {
  ...
  var queryParser: MultiFieldQueryParser? = null

  var query: Query? = null

  @RequiresApi(Build.VERSION_CODES.O) // TODO
  fun resourceSearcher(indexDirPath: String) {
    val indexDir: Directory = FSDirectory.open(File(indexDirPath).toPath())
    reader = DirectoryReader.open(indexDir)
    indexSearcher = IndexSearcher(reader)
    val matchFields =
      arrayOf<String>(Patient.SP_ACTIVE, Patient.SP_ADDRESS_CITY, Patient.SP_ADDRESS_COUNTRY)
    queryParser = MultiFieldQueryParser(matchFields, StandardAnalyzer())
  }
