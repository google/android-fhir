...
object R4Sync {
  ...
  suspend fun oneTimeSync(
    context: Context,
    fhirEngine: R4FhirEngine,
    downloadManager: R4DownloadWorkManager,
    resolver: ConflictResolver
  ): Result {
    return R4FhirEngineProvider.getDataSource(context)?.let {
      R4FhirSynchronizer(context, fhirEngine, it, downloadManager, conflictResolver = resolver)
        .synchronize()
    }
      ?: Result.Error(
        listOf(
   ...
   */
  inline fun <reified W : R4FhirSyncWorker> oneTimeSync(
    context: Context,
   ...
   */
  inline fun <reified W : R4FhirSyncWorker> periodicSync(
    context: Context,
  ...
  @PublishedApi
  internal inline fun <W : R4FhirSyncWorker> createOneTimeWorkRequest(
  ...
  @PublishedApi
  internal inline fun <W : R4FhirSyncWorker> createPeriodicWorkRequest(
    periodicSyncConfiguration: PeriodicSyncConfiguration,
    clazz: Class<W>
  ): PeriodicWorkRequest {
