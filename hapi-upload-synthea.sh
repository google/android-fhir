...
./run_synthea -m pregnancy -p "$POPULATION" -s 12345 --exporter.fhir.included_resources Patient --exporter.years_of_history 1 --exporter.fhir.use_us_core_ig false --exporter.fhir.transaction_bundle true --exporter.practitioner.fhir.export true --exporter.hospital.fhir.export true > /dev/null

cd - || exit
# Upload hospital information
for filename in "$SYNTHEA_DIR"/output/fhir/hospital*.json; do
  curl "$SERVER_URL"/fhir --data-binary "@$filename" -H "Content-Type: application/fhir+json"
done
