...
val r4FhirAdapter =
  FhirAdapter.builder(FhirVersionEnum.R4)
    .setEvaluateFunction { resource, path ->
      FHIRPathEngine(SimpleWorkerContext()).evaluate(resource as Base, path)
    }
    .setHasLastUpdate { (it as Meta).hasLastUpdated() }
    .setToLastUpdatedElement { (it as Meta).lastUpdatedElement.value }
    .setHasProfile { (it as Meta).hasProfile() }
    .setHasTag { (it as Meta).hasTag() }
    .setToNumberIndex { searchParam, value ->
      when (value.fhirType()) {
        "integer" ->
    ...
    .setToDateIndex { searchParam, value ->
    ...
    .setToDateTimeIndex { searchParam, value ->
      when (value.fhirType()) {
        "dateTime" -> {
    ...
    .setToStringIndex { searchParam, value ->
      if (!value.isEmpty) {
        StringIndex(
          searchParam.name,
          searchParam.path,
    ...
    .setToTokenIndexList { searchParam, value ->
      when (value.fhirType()) {
        "boolean" ->
          listOf(
            TokenIndex(
    ...
    .setToReferenceIndex { searchParam, value ->
      when (value) {
    ...
    .setToQuantityIndexList { searchParam, value ->
      when (value.fhirType()) {
        "Money" -> {
    ...
    .setToUriIndex { searchParam, value ->
    ...
    .setToDate { value -> DateType(value) }
    .setToDateTime { value -> DateTimeType(value) }
    .setToGetConditionParamPairForDateTimeType { prefix, value ->
    ...
    .setToGetConditionParamPairForDateType { prefix, value ->
    ...
    .setValidateResourceType { value -> ResourceType.fromCode(value) }
    .build()
