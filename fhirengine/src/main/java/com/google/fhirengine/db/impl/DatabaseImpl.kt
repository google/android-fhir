...
internal class DatabaseImpl @Inject constructor(
    ...
    override fun <R : Resource?> searchByReference(clazz: Class<R>, reference: String,
                                                   value: String): List<R>? {
        return dao.getResourceByReferenceIndex(ResourceUtils.getResourceType(clazz).name, reference, value).map { iParser.parseResource(it) as R }
    }

    override fun <R : Resource?> searchByString(clazz: Class<R>?, string: String,
                                                value: String): List<R> {
        return dao.getResourceByStringIndex(ResourceUtils.getResourceType(clazz).name, string, value).map { iParser.parseResource(it) as R }
    }

    override fun <R : Resource?> searchByCode(clazz: Class<R>?, code: String, system: String,
                                                value: String): List<R> {
        return dao.getResourceByCodeIndex(ResourceUtils.getResourceType(clazz).name, code, system, value).map { iParser.parseResource(it) as R }
    }

    override fun <R : Resource?> searchByReferenceAndCode(clazz: Class<R>, reference: String,
                                                          refvalue: String, string: String, system: String,
                                                          value: String): List<R>? {
