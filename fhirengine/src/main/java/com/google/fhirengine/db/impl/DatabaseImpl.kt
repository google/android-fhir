...
//        resourceDao.allLocalChanges()
//                .groupBy({ it.resourceType.name + "/" + it.resourceId })
//                .map { resourceDao.squash(it.value) }
//                .map { Update(
...
//                ) }
    ...
    override fun deleteLocalChanges(resourceId: String, resourceType: String) {
        resourceDao.deleteLocalChanges(resourceId, resourceType)
    }
}
