...
/**
 * Indexes a FHIR resource according to the [search parameters](https://www.hl7.org/fhir/searchparameter-registry.html).
 */
internal object ResourceIndexer {
    private val context = FhirContext.forR4()
    private val fhirPathEngine = FHIRPathEngine(
        HapiWorkerContext(context, DefaultProfileValidationSupport(context))
    )
    ...
    private fun <R : Resource> extractIndexValues(resource: R): ResourceIndices {
        ...
        resource.javaClass.fields.asSequence().mapNotNull {
            ...
            it to fhirPathEngine.evaluate(resource, it.path)
        }.flatMap { pair ->
            pair.second.map { pair.first to it }
        }.forEach { pair ->
            val (searchParam, value) = pair
            when (SearchParamType.fromCode(pair.first.type)) {
                SearchParamType.NUMBER -> numberIndex(searchParam, value)?.also {
                    indexBuilder.addNumberIndex(it)
                }
                SearchParamType.DATE -> dateIndex(searchParam, value)?.also {
                    indexBuilder.addDateIndex(it)
                }
                SearchParamType.STRING -> stringIndex(searchParam, value)?.also {
                    indexBuilder.addStringIndex(it)
                }
                SearchParamType.TOKEN -> tokenIndex(searchParam, value).forEach {
                    indexBuilder.addTokenIndex(it)
                }
                SearchParamType.REFERENCE -> referenceIndex(searchParam, value)?.also {
                    indexBuilder.addReferenceIndex(it)
                }
                SearchParamType.QUANTITY -> quantityIndex(searchParam, value)?.also {
                    indexBuilder.addQuantityIndex(it)
                }
    ...
    private fun numberIndex(searchParam: SearchParamDefinition, value: Base): NumberIndex? =
        when (value.fhirType()) {
            "integer" -> NumberIndex(
                searchParam.name,
                searchParam.path,
                BigDecimal((value as IntegerType).value)
            )
            "decimal" -> NumberIndex(
                searchParam.name,
                searchParam.path,
                (value as DecimalType).value
            )
            else -> null
        }
    ...
    private fun tokenIndex(searchParam: SearchParamDefinition, value: Base): List<TokenIndex> =
        when (value.fhirType()) {
            ...
            "CodeableConcept" -> {
                ...
                codeableConcept.coding.filter {
                    it.code != null && it.code!!.isNotEmpty()
                }.map {
