...
class ActivityFlowTest {
  ...
  fun `order medication flow for medication dispense`(): Unit = runBlockingOnWorkerThread {
    ...
    flow.draftPlan().onSuccess {
      draftPlan ->
      draftPlan.update {
        addNote(Annotation(MarkdownType("Draft plan looks OK.")))
      }

      flow.startPlan(draftPlan).onSuccess {
        planPhase ->
        val updatedPlan = planPhase.getRequest().copy().apply {
          setStatus(Status.ACTIVE)
          update {
            addNote(Annotation(MarkdownType("Plan looks OK.")))
          }
        }

        planPhase.update(updatedPlan).onSuccess {
    ...
    val planToResume = repository.read(MedicationRequest::class.java, IdType("MedicationRequest", cachedPlanId)).let {
      CPGMedicationRequest(it)
    }
  ...
  resumedPlanFlow.draftOrder().onSuccess {
      draftOrder ->
    draftOrder.update {
      addNote(Annotation(MarkdownType("Draft order looks OK.")))
    }

    resumedPlanFlow.startOrder(draftOrder).onSuccess {
        orderPhase ->
      val updatedOrder = orderPhase.getRequest().copy().apply {
        setStatus(Status.ACTIVE)
        update {
          addNote(Annotation(MarkdownType("Order looks OK.")))
        }
      }

      orderPhase.update(updatedOrder).onSuccess {
        cachedPlanId = updatedOrder.logicalId
      }.onFailure {
        fail("Should have succeeded", it)
      }
    }
  ...
  val orderToResume = repository.read(MedicationRequest::class.java, IdType("MedicationRequest",cachedPlanId)).let {
    CPGMedicationRequest(it)
  }
  ...
  resumedOrderFlow.draftPerform(CPGMedicationDispenseEvent::class.java).onSuccess {
      draftEvent ->

    draftEvent.update {
      addNote(Annotation(MarkdownType("Draft event looks OK.")))
    }

    resumedOrderFlow.startPerform(draftEvent).onSuccess {
        performPhase ->
      val updatedEvent = performPhase.getEvent().copy().apply {
        setStatus(EventStatus.INPROGRESS)

        update {
          addNote(Annotation(MarkdownType("Event looks OK.")))
        }
      }

      performPhase.update(updatedEvent).onSuccess {
        cachedPlanId = updatedEvent.logicalId
      }.onFailure {
        fail("Should have succeeded", it)
      }
    }
    ...
    fail("Unexpected",it)
     ...
     val eventToResume = repository.read(MedicationDispense::class.java, IdType("MedicationDispense",cachedPlanId)).let {
       CPGMedicationDispenseEvent(it)
    }

  val resumedPerformFlow = ActivityFlow.of(repository, eventToResume)
  assertThat(resumedPerformFlow.getCurrentPhase().getPhaseName()).isEqualTo(Phase.PhaseName.PERFORM)

    assertThat(resumedPerformFlow.draftPlan().isFailure).isTrue()
    assertThat(resumedPerformFlow.draftOrder().isFailure).isTrue()
    assertThat(resumedPerformFlow.draftPerform(CPGMedicationDispenseEvent::class.java).isFailure).isTrue()

  (resumedPerformFlow.getCurrentPhase() as Phase.EventPhase<*>).complete()

  val completedEvent = repository.read(MedicationDispense::class.java, IdType("MedicationDispense",cachedPlanId)).let {
    CPGMedicationDispenseEvent(it)
  }
  ...
  fun `communication request flow new api`(): Unit = runBlockingOnWorkerThread {
    ...
    val draftPlanResult = flow.draftPlan().
    onSuccess {
      it.update {
        addNote().apply {
          text = "This Draft Plan looks OK, so approving it."
        }
      }
    }.onFailure {
      throw it
    ...
    newPhase.onSuccess {
      ...
      request.update {
        addNote().apply {
          text = "This Plan looks OK, so marking it active."
        }
      }
      ...
      planPhase.update(request).onSuccess {
       ...
       // Get latest request from the phase.
        ...
        val draftOrder = flow.draftOrder().onSuccess {
          it.update {
            addNote().apply {
              text = "This Draft Order looks OK, so approving it."
            }
          }
        }
        ...
        newPhase.onSuccess { orderPhase ->
          ...
          order.update {
            addNote().apply {
              text = "This Order looks OK, so marking it active."
            }
          }
          ...
          orderPhase.getRequest().resource.note.forEachIndexed {
              a,b->
            println("Note $a. ${b.text}")
          }

          val draftEvent = flow.draftPerform(CPGCommunicationEvent::class.java).onSuccess {
            it.update {
              addNote().apply {
                text = "This Event looks OK, so approving it."
              }
            }
          }
          ...
          newPhase.onSuccess {
            ...
            event.update {
              addNote().apply {
                text = "This Event looks OK, so marking it active."
              }
            }
            ...
            phase.getEvent().resource.note.forEachIndexed {
                a,b->
              println("Note $a. ${b.text}")
            }
          }
