...
class ActivityFlow4<R : CPGRequestResource<*>, E : CPGEventResource<*>>
internal constructor(
  ...
  override suspend fun startPlan(init: R.() -> Unit): EndPlan<R, E> {
    ...
    return ActivityFlow4(repository, startPlan(requestResource), null)
  }
  ...
  override suspend fun startOrder(init: R.() -> Unit): EndOrder<R, E> {
    requestResource.init()
    return ActivityFlow4(repository, startOrder(requestResource), null)
  }
  ...
  companion object {
    ...
    ): CPGCommunicationActivity = ActivityFlow4(repository, resource)

    // Collect information
    fun of(repository: Repository, resource: CPGTaskRequest) =
      ActivityFlow4(repository, resource, null)
    ...
    fun of(
      repository: Repository,
      patientId: String,
    ): List<ActivityFlow4<CPGRequestResource<*>, CPGEventResource<*>>> {
      val tasks =
        repository
          .search(
            Bundle::class.java,
      ...
      return requestChain.map { ActivityFlow4(repository, it.request) }
    }
  }
}
