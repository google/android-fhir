...
/**
 * A simple [Fragment] subclass. Use the [ScreeningsViewPagerFragment.newInstance] factory method to
...
class ScreeningsViewPagerFragment : Fragment(), ScreeningClickHandler {
  ...
  class ScreeningsViewPagerAdapter(private val fragment: Fragment) :
    FragmentStateAdapter(fragment) {
    override fun getItemCount() = 2

    override fun createFragment(position: Int): Fragment {
      val listScreeningsFragment =
        ListScreeningsFragment((fragment as ScreeningClickHandler)).apply {
          arguments =
            bundleOf(
              ListScreeningsFragment.TAB_POSITION to position,
              ListScreeningsFragment.PATIENT_ID_KEY to
                (fragment as ScreeningsViewPagerFragment).args.patientId
            )
        }
