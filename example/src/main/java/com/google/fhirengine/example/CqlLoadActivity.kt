...
import com.google.fhirengine.example.FhirApplication.Companion.fhirEngine
...
class CqlLoadActivity : AppCompatActivity() {
    ...
    private inner class DownloadFhirResource :
    ...
    private inner class EvaluateAncLibrary :
        AsyncTask<String?, String?, EvaluationResult>() {
        ...
        override fun onPostExecute(result: EvaluationResult) {
            ...
            for (libraryResult in result.libraryResults.values) {
                for ((key, value) in libraryResult.expressionResults) {
                    ...
                    } else if (MutableList::class.java.isAssignableFrom(
                            value.javaClass)) {
                        for (listItem in value as List<*>) {
                            stringBuilder.append(
                                FhirContext.forR4().newJsonParser().encodeResourceToString(
                                    listItem as Resource?))
                        }
                    } else if (Resource::class.java.isAssignableFrom(
                            value.javaClass)) {
                        stringBuilder.append(
                            FhirContext.forR4().newJsonParser()
                                .encodeResourceToString(value as Resource))
                    } else {
