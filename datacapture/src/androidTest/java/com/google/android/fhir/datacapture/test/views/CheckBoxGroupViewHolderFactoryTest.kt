...
import com.google.android.fhir.datacapture.views.components.CHOICE_CHECKBOX_IMAGE_TAG
...
import com.google.android.fhir.datacapture.views.factories.CHECKBOX_OPTION_TAG
...
class CheckBoxGroupViewHolderFactoryTest {
  @get:Rule
  ...
  fun setUp() {
    activityScenarioRule.scenario.onActivity { activity ->
      viewHolder = CheckBoxGroupViewHolderFactory.create(FrameLayout(activity))
  ...
  fun bind_vertical_shouldCreateCheckBoxButtons() {
    ...
    composeTestRule.onAllNodes(hasTestTag(CHECKBOX_OPTION_TAG)).assertCountEquals(2)
    questionnaire.answerOption.forEachIndexed { index, opt ->
      composeTestRule
        .onAllNodes(hasTestTag(CHECKBOX_OPTION_TAG))[index]
        .assertTextEquals(opt.valueCoding.display)

      val node = composeTestRule.onAllNodes(hasTestTag(CHECKBOX_OPTION_TAG))[index]
  ...
  fun bind_noAnswer_shouldLeaveCheckButtonsUnchecked() {
    ...
    composeTestRule.onNode(hasTestTag(CHECKBOX_OPTION_TAG) and hasText("Coding 1")).assert(isOff())
  }

  @Test
  fun bind_withImageInItemAnswerMediaExtension_shouldShowImageAfterCheckBox() {
    viewHolder.bind(
      QuestionnaireViewItem(
        Questionnaire.QuestionnaireItemComponent().apply {
          repeats = true
    ...
    composeTestRule
      .onNodeWithTag(CHOICE_CHECKBOX_IMAGE_TAG, useUnmergedTree = true)
      .assertIsDisplayed()
  }
  ...
  fun bind_answer_shouldSetCheckBoxButton() {
    ...
    composeTestRule.onNode(hasTestTag(CHECKBOX_OPTION_TAG) and hasText("Coding 1")).assert(isOn())
  }
  ...
  fun click_shouldAddQuestionnaireResponseItemAnswer() {
    ...
    composeTestRule.onNode(hasTestTag(CHECKBOX_OPTION_TAG) and hasText("Coding 1")).performClick()
  ...
  fun optionExclusiveAnswerOption_click_deselectsOtherAnswerOptions() {
    ...
    viewHolder.bind(questionnaireViewItem)
    composeTestRule.onNode(hasTestTag(CHECKBOX_OPTION_TAG) and hasText("display-2")).performClick()
    composeTestRule.waitUntil { answersUpdated }
    answersUpdated = false
    composeTestRule.onNode(hasTestTag(CHECKBOX_OPTION_TAG) and hasText("display-1")).performClick()
    composeTestRule.waitUntil { answersUpdated }
  ...
  fun bind_readOnly_shouldDisableView() {
    ...
    composeTestRule
      .onNode(hasTestTag(CHECKBOX_OPTION_TAG) and hasText("Coding 1"))
      .assert(isNotEnabled())
  }
  ...
  fun hideRequiredText() {
    ...
    composeTestRule.onNodeWithTag(REQUIRED_OPTIONAL_HEADER_TEXT_TAG).assertIsNotDisplayed()
  }
