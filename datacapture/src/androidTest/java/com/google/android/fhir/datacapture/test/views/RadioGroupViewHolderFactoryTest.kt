...
import com.google.android.fhir.datacapture.views.factories.RADIO_OPTION_TAG
...
class RadioGroupViewHolderFactoryTest {
  ...
  fun setUp() {
    activityScenarioRule.scenario.onActivity { activity ->
      viewHolder = RadioGroupViewHolderFactory.create(FrameLayout(activity))
  ...
  fun bind_vertical_shouldCreateRadioButtons() {
    ...
    composeTestRule.onAllNodes(hasTestTag(RADIO_OPTION_TAG)).assertCountEquals(2)
    questionnaire.answerOption.forEachIndexed { index, opt ->
      composeTestRule
        .onAllNodes(hasTestTag(RADIO_OPTION_TAG))[index]
        .assertTextEquals(opt.valueCoding.display)

      val node = composeTestRule.onAllNodes(hasTestTag(RADIO_OPTION_TAG))[index]
      node.assertTextEquals(opt.valueCoding.display)
      ...
      assertThat(nodeWidth).isEqualTo(parentWidth)
    }
  }
  ...
  fun bind_horizontal_shouldCreateRadioButtons() {
    ...
    questionnaire.answerOption.forEachIndexed { index, opt ->
      ...
      val parentWidth =
        semanticsNode.layoutInfo.coordinates.parentLayoutCoordinates?.size?.width ?: 0
      assertThat(nodeWidth).isLessThan(parentWidth)
    }
  }
  ...
  fun bind_noAnswer_shouldLeaveRadioButtonsUnchecked() {
    ...
    composeTestRule
      .onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 1"))
      .assertIsNotSelected()
  }
  ...
  fun bind_withImageInItemAnswerMediaExtension_shouldShowImageAfterRadioButtons() {
    ...
    composeTestRule
      .onNodeWithTag(CHOICE_RADIO_BUTTON_IMAGE_TAG, useUnmergedTree = true)
      .assertIsDisplayed()
  }
  ...
  fun bind_answer_shouldCheckRadioButton() {
    ...
    composeTestRule.onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 1")).assertIsSelected()

    composeTestRule
      .onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 2"))
      .assertIsNotSelected()
  }
  ...
  fun click_shouldSetQuestionnaireResponseItemAnswer() {
    ...
    composeTestRule.onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 1")).performClick()
  ...
  fun click_shouldCheckOtherRadioButton() {
    ...
    composeTestRule.onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 2")).performClick()

    composeTestRule
      .onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 1"))
      .assertIsNotSelected()
    composeTestRule.onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 2")).assertIsSelected()
  }

  @Test
  fun unselectRadioButtonIfSelectedRadioButtonIsClicked() {
    val questionnaireViewItem =
      QuestionnaireViewItem(
        Questionnaire.QuestionnaireItemComponent().apply {
          addAnswerOption(
  ...
  @Test
  fun clearTheAnswerIfSelectedRadioButtonIsClicked() {
    var answersHolder: List<QuestionnaireResponse.QuestionnaireResponseItemAnswerComponent>? = null
    ...
    viewHolder.bind(questionnaireViewItem)
    composeTestRule.onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 1")).performClick()
    composeTestRule.waitUntil { answerChanged }
    assertThat(answersHolder!!.isNotEmpty()).isTrue()

    answerChanged = false
    composeTestRule.onNode(hasTestTag(RADIO_OPTION_TAG) and hasText("Coding 1")).performClick()
    composeTestRule.waitUntil { answerChanged }

    assertThat(answersHolder.isEmpty()).isTrue()
  }
  ...
  fun bind_readOnly_shouldDisableView() {
    ...
    composeTestRule.onAllNodes(hasTestTag(RADIO_OPTION_TAG)).assertAll(isNotEnabled())
  }
  ...
  fun showRequiredText() {
    ...
    composeTestRule.onNodeWithTag(REQUIRED_OPTIONAL_HEADER_TEXT_TAG).assertTextEquals("Required")
  }
  ...
  fun showOptionalText() {
    ...
    composeTestRule.onNodeWithTag(REQUIRED_OPTIONAL_HEADER_TEXT_TAG).assertTextEquals("Optional")
  }
