...
private fun Base.addRepeatedFieldValue(fieldName: String) =
  javaClass
    .getMethod(
      "add${fieldName.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.ROOT) else it.toString() }}"
    )
    .invoke(this) as Base
...
private fun Base.getChoiceFieldValue(fieldName: String) =
  javaClass
    .getMethod(
      "get${fieldName.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.ROOT) else it.toString() }}"
    )
    .invoke(this) as Base
...
private fun updateFieldWithEnum(base: Base, field: Field, value: Base) {
  ...
  base.javaClass
    .getMethod(
      "set${field.name.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.ROOT) else it.toString() }}",
      field.nonParameterizedType
    )
...
private fun setFieldElementValue(base: Base, field: Field, answerValue: Base) {
  base.javaClass
    .getMethod(
      "set${field.name.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.ROOT) else it.toString() }}Element",
      field.type
    )
