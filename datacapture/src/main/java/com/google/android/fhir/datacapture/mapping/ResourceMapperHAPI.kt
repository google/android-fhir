...
object ResourceMapperHAPI {
    ...
    fun extract(
        ...
        val resource = Class.forName("com.google.android.fhir.datacapture.model.$className").newInstance() as Resource
        ...
        while (
            ...
            if(questionnaireItem.type == Questionnaire.QuestionnaireItemType.GROUP) {
                val innerClass = Class.forName("org.hl7.fhir.r4.model.${questionnaireItem.itemContextNameToExpressionMap2.values.first()}")
                ...
                resource.javaClass.getMethod("add${targetFieldName.capitalize()}", innerClass).invoke(
                        resource,
                        type
                )
                continue
            ...
            questionnaireItem.type.getClass()?.let {
                resource.javaClass.getMethod("set${targetFieldName.capitalize()}Element", it).invoke(
                        resource,
    ...
    private fun createInnerClassObject(type: Type, questionnaireItemComponent: MutableList<Questionnaire.QuestionnaireItemComponent>,
                                       questionnaireResponseItemComponent: MutableList<QuestionnaireResponse.QuestionnaireResponseItemComponent>) {
        var questionnaireItemListIterator2 = questionnaireItemComponent.iterator()
        var questionnaireResponseItemListIterator2 = questionnaireResponseItemComponent.iterator()

        while (
                questionnaireItemListIterator2.hasNext() &&
            ...
            try {
                questionnaireItem2.type.getClass()?.let {
                    type.javaClass.getMethod("set${targetFieldName2.capitalize()}Element", it).invoke(
                            type,
                            questionnaireResponseItem2.answer.first().value
                    )
                }
            } catch (e: NoSuchMethodException) {
                questionnaireItem2.type.getClass()?.let {
                    type.javaClass.getMethod("set${targetFieldName2.capitalize()}", List::class.java).invoke(
                            type,
                            listOf(questionnaireResponseItem2.answer.first().value)
                    )
                }
            }
...
private fun Questionnaire.QuestionnaireItemType.getClass() = when (this) {
...
private val Questionnaire.itemContextNameToExpressionMap: Map<String, String>
    get() {
        return this.extension.filter {
            it.url == ITEM_CONTEXT_EXTENSION_URL
        }.map {
...
private val Questionnaire.QuestionnaireItemComponent.itemContextNameToExpressionMap2: Map<String, String>
    get() {
        return this.extension.filter {
            it.url == ITEM_CONTEXT_EXTENSION_URL
        }.map {
...
private const val ITEM_CONTEXT_EXTENSION_URL: String = "http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-itemContext"