...
2.  We'll upload test data for three patients to the local HAPI FHIR server. Run
...
For your convenience, the FHIR Engine Library dependencies have already been add
to the project. This allows you to integrate the FHIR Engine Library in your
app. Observe the following lines to the end of the `app/build.gradle.kts` file
of your project:
...
2.  Inside the `onCreate()` method, add the following code to initialize FHIR
    Engine: `kotlin FhirEngineProvider.init( FhirEngineConfiguration(
    enableEncryptionIfSupported = true, RECREATE_AT_OPEN, ServerConfiguration(
    baseUrl = "http://10.0.2.2:8080/fhir/", httpLogger = HttpLogger(
    HttpLogger.Configuration( if (BuildConfig.DEBUG) HttpLogger.Level.BODY else
    HttpLogger.Level.BASIC ) ) { Log.d("App-HttpLog", it) }, ), ) )`

    Notes: * enableEncryptionIfSupported: Enables data encryption if the device
    supports it. * RECREATE_AT_OPEN: Determines the database error strategy. In
...
3.  In the FhirApplication class, add the following line to lazily instantiate
...
4.  Add the following convenience methods in the `FhirApplication` class for
...
This static method allows you to retrieve the FHIR Engine instance from anywhere
...
responses and extracts the resources from the returned bundle, which get saved
...
Here, we've defined which download manager, conflict resolver, and FHIR engine
instance to use for syncing.
...
AppFhirSyncWorker we defined earlier. It will then update the UI based on the
...
Here, when the sync process finishes, a toast message will display notifying the
user, and the app will then display all patients by invoking a search with an
empty name.
...
2.  **Search for Patients from Wakefield**:
...
3.  **Search for Patients from Taunton**:
...
2.  **Search for Patients**: On the patient list screen, use the search
