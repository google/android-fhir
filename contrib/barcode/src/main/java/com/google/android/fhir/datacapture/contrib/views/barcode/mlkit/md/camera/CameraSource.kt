...
class CameraSource(private val graphicOverlay: GraphicOverlay) {
  ...
  internal fun start(surfaceHolder: SurfaceHolder) {
    ...
    processingThread =
      Thread(processingRunnable).apply {
   ...
   *
   * Call [.release] instead to completely shut down this camera source and release the resources of
  ...
  private fun setRotation(camera: Camera, parameters: Parameters) {
    ...
    val degrees =
      when (val deviceRotation = windowManager.defaultDisplay.rotation) {
  ...
  private inner class FrameProcessingRunnable internal constructor() : Runnable {
     ...
     *
     * If detection takes longer than the time in between new frames from the camera, this will mean
     * that this loop will run without ever waiting on a frame, avoiding any context switching or
     * frame acquisition time latency.
     *
     * If you find that this is using more CPU than you'd like, you should probably decrease the FPS
     * setting above to allow for some idle time in between frames.
     */
    override fun run() {
      var data: ByteBuffer?
      ...
      while (true) {
        ...
        try {
          synchronized(processorLock) {
            val frameMetadata =
              FrameMetadata(previewSize!!.width, previewSize!!.height, rotationDegrees)
