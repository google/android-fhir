 ...
 *
 * Supports scaling and mirroring of the graphics relative the camera's preview properties. The idea
 * is that detection items are expressed in terms of a preview size, but need to be scaled up to the
 * full view size, and also mirrored in the case of the front-facing camera.
 *
...
class GraphicOverlay(context: Context, attrs: AttributeSet) : View(context, attrs) {
   ...
   * this and implement the [Graphic.draw] method to define the graphics element. Add instances to
   * the overlay using [GraphicOverlay.add].
   */
  ...
  /**
   * Adjusts the `rect`'s coordinate from the preview's coordinate system to the view coordinate
   * system.
   */
  fun translateRect(rect: Rect) =
    RectF(
  ...
  override fun onDraw(canvas: Canvas) {
    ...
    synchronized(lock) { graphics.forEach { it.draw(canvas) } }
  }
}
