 ...
 * submitting the [Questionnaire]. This is shown when user has pressed internal submit button in the
 * [com.google.android.fhir.datacapture.QuestionnaireFragment] and there are some validation errors.
 */
...
internal class QuestionnaireValidationErrorViewModel : ViewModel() {
  ...
  fun getItemsTextWithValidationErrors(): List<AnnotatedString> {
    ...
    return questionnaire
      ?.item
      ?.flattened()
      ?.filter { invalidFields.contains(it.linkId.value) }
      ?.mapNotNull {
