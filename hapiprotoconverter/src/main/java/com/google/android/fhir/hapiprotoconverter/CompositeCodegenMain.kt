    ...
    CompositeCodegen.profileUrlMap.values  //.filter { it.name.value == "Extension"}
        .forEach { def ->
            ...
            // def.status.value == PublicationStatusCode.Value.ACTIVE
                ...
                try {
                    Class.forName(ClassName(CompositeCodegen.hapiPackage,def.id.value.capitalize()).reflectionName())
                    Class.forName(ClassName(CompositeCodegen.protoPackage,def.id.value.capitalize()).reflectionName())
