...
public object PaymentReconciliationConverter {
  public fun PaymentReconciliation.toHapi(): org.hl7.fhir.r4.model.PaymentReconciliation {
    ...
    hapiValue.setStatus(org.hl7.fhir.r4.model.PaymentReconciliation.PaymentReconciliationStatus.valueOf(status.value.name.replace("_","")))
    ...
    hapiValue.setNotes(notesList.map{it.toHapi()})
    return hapiValue
  }
  ...
  public fun org.hl7.fhir.r4.model.PaymentReconciliation.toProto(): PaymentReconciliation {
    ...
    .setStatus(PaymentReconciliation.StatusCode.newBuilder().setValue(FinancialResourceStatusCode.Value.valueOf(status.toCode().replace("-",
        "_").toUpperCase())).build())
    ...
    .setOutcome(PaymentReconciliation.OutcomeCode.newBuilder().setValue(ClaimProcessingCode.Value.valueOf(outcome.toCode().replace("-",
        "_").toUpperCase())).build())
    ...
    .addAllNotes(notes.map{it.toProto()})
    .build()
    return protoValue
  }

  public fun org.hl7.fhir.r4.model.PaymentReconciliation.DetailsComponent.toProto():
  ...
  public fun org.hl7.fhir.r4.model.PaymentReconciliation.NotesComponent.toProto():
      PaymentReconciliation.Notes {
    val protoValue = PaymentReconciliation.Notes.newBuilder()
    .setId(String.newBuilder().setValue(id))
    .addAllExtension(extension.map{it.toProto()})
    .addAllModifierExtension(modifierExtension.map{it.toProto()})
    .setType(PaymentReconciliation.ProcessNote.TypeCode.newBuilder().setValue(NoteTypeCode.Value.valueOf(type.toCode().replace("-",
        "_").toUpperCase())).build())
  ...
  public fun PaymentReconciliation.Details.toHapi():
  ...
  public fun PaymentReconciliation.Notes.toHapi():
