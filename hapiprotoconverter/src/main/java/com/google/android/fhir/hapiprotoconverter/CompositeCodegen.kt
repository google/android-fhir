...
import com.google.fhir.r4.core.MimeType
...
object CompositeCodegen {
    ...
    private val specialValueSet = listOf(
    ...
    private const val uriBindingName= "http://hl7.org/fhir/StructureDefinition/elementdefinition-bindingName"
    ...
    fun generate(def: StructureDefinition, outLocation: File? = null) {
        ...
        // respectively
        val backboneElementMap =
            mutableMapOf<kotlin.String, Pair<FunSpec.Builder, FunSpec.Builder>>()
        ...
        for (element in def.snapshot.elementList) {
            ...
            if (element.typeList.size > 1) {
                functionsList.addAll(handleChoiceType(element,getToHapiBuilder(element), getToProtoBuilder(element),fileBuilder))
                continue
            }
            if (elementName == "id") {
                getToProtoBuilder(element)
                    .addStatement(".setId(%T.newBuilder().setValue(id))", String::class)
        ...
        fileBuilder
            .addType(
                TypeSpec.objectBuilder(complexConverterClass.simpleName).addFunctions(functionsList)
                    .build()
            )
            .build()
            .writeTo(outLocation!!)
    ...
    private fun getCodeSystemName(name: String): kotlin.String {
        val filteredName = name.value.replace("[^A-Za-z0-9]".toRegex(), "")
    ...
    private fun getValueSetName(name: String): kotlin.String {
        val filteredName = name.value.replace("[^A-Za-z0-9]", "")
    ...
    private fun getElementName(element: ElementDefinition): kotlin.String {
        // TODO change logic
        return if (element.extensionList.any { it.url.value == explicitTypeName }) {
            element.extensionList.single { it.url.value == explicitTypeName }.value.stringValue.value
        } else {
        ...
        if (isSingle) {
            protoBuilder.addStatement(
                ...
                if (element.typeList.first().profileList.isNotEmpty())
                    ClassName(
                        hapiPackage, normalizeType(element.typeList.first())
                    ) else ""
            )
            ...
            protoBuilder.addStatement(
                ...
                elementName,
                if (element.typeList.first().profileList.isNotEmpty())
                    ClassName(hapiPackage, normalizeType(element.typeList.first())) else ""
            )
            hapiBuilder.addStatement(
    ...
    private fun handleChoiceType(
        ...
        protoBuilder.addStatement("$singleMethodTemplate(%L.%N())", getElementName(element).removeSuffix(choiceTypeSuffixStructureDefinition).capitalizeFirst(),
            getElementName(element).removeSuffix(choiceTypeSuffixStructureDefinition),elementToProtoBuilder.build())
        hapiBuilder.addStatement("hapiValue$singleMethodTemplate(%L.%N())", getElementName(element).removeSuffix(choiceTypeSuffixStructureDefinition).capitalizeFirst(),getElementName(element).removeSuffix(choiceTypeSuffixStructureDefinition),elementToHapiBuilder.build())
    ...
    private fun handleBackBoneElementTypes(
        ...
        if (isSingle) {
            protoBuilder.addStatement(
                ...
                elementName,
                if (element.typeList.first().profileList.isNotEmpty())
                    normalizeType(element.typeList.first()) else ""
            )
            hapiBuilder.addStatement(
                "hapiValue$singleMethodTemplate(%L.toHapi())",
                elementName.capitalizeFirst(),
    ...
    private fun ElementDefinition.getBackBoneProtoClass(): ClassName {
        return ClassName(protoPackage, this.base.path.value.split(".").dropLast(1))
            .nestedClass(getElementName(this).capitalizeFirst())
    }

    private fun ElementDefinition.getBackBoneHapiClass(): ClassName {
        return ClassName(hapiPackage, this.base.path.value.split(".").dropLast(1))
            .nestedClass(
                "${
                    this.base.path.value.split(".").first()
    ...
    private fun getHapiProtoConverterFuncPair(
        ...
        return FunSpec.builder("${funcName}ToProto".lowerCaseFirst())
            ...
            .returns(protoClass) to
                FunSpec.builder("${funcName}ToHapi".lowerCaseFirst()).receiver(protoClass)
                    .returns(hapiClass)
    }

    private fun CharSequence.lowerCaseFirst() = this[0].toLowerCase().toString() + this.drop(1)
...
fun main() {
    File("C:\\Users\\Aditya\\Desktop\\fhir-spec\\site").listFiles()!!
        .filter {
            it.name.startsWith("valueset-") &&
                    ...
                    !it.name.startsWith("valueset-questionnaire")
                    && it.name.endsWith(".json")
        }
        .forEach {
            try {
                val def =
                    JsonFormat.getParser().merge(it.inputStream().reader(), ValueSet.newBuilder())
    ...
    File("hapiprotoconverter\\sampledata\\").listFiles()!!
        ...
        .forEach {
            val def = JsonFormat.getParser()
                .merge(it.inputStream().reader(), StructureDefinition.newBuilder())
            CompositeCodegen.profileUrlMap[def.url.value] = def.build()
        }
    CompositeCodegen.profileUrlMap.values // .filter { it.name.value == "Dosage" }
        .forEach { def ->
