...
public object EndpointConverter {
  public fun Endpoint.toHapi(): org.hl7.fhir.r4.model.Endpoint {
    ...
    hapiValue.setStatus(org.hl7.fhir.r4.model.Endpoint.EndpointStatus.valueOf(status.value.name.replace("_","")))
    ...
    payloadMimeTypeList.map{hapiValue.addPayloadMimeType(it)}
  ...
  public fun org.hl7.fhir.r4.model.Endpoint.toProto(): Endpoint {
    ...
    .setStatus(Endpoint.StatusCode.newBuilder().setValue(EndpointStatusCode.Value.valueOf(status.toCode().replace("-",
        "_").toUpperCase())).build())
    ...
    .addAllPayloadMimeType(payloadMimeType.map{Endpoint.PayloadMimeTypeCode.newBuilder().setValue(it).build()})
